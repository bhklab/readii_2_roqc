...
@click.option('--nifti_dir', type=click.Path(exists=True, file_okay=False, dir_okay=True), default=None, help='Path to the directory containing the nifti images and masks to process. If not provided, will use the default path of data/procdata/{datasource}_{dataset}/images/mit_{dataset}.')
...
def generate_dataset_index(dataset: str, 
    ...
    nifti_dir:Path | None
        Path to the directory containing the nifti images and masks to process. If not provided, will use the default path of data/procdata/{datasource}_{dataset}/images/mit_{dataset}.
    overwrite:bool = False
    ...
    nifti_dir = dirs.PROCDATA / full_data_name / "images" / f'mit_{dataset_name}' if nifti_dir is None else Path(nifti_dir)

    # Load imgtools autopipeline index file
    mit_index_path = nifti_dir / f'{nifti_dir.stem}_index-simple.csv'
    ...
    regions, permutations, crop, resize = get_readii_settings(dataset_config)
    if any(setting is not None for setting in (regions, permutations, crop, resize)):
        if all(setting is not None for setting in (regions,permutations)):
            ...
            readii_index_path = get_readii_index_filepath(dataset_config,
                                                        readii_image_dir = nifti_dir.parent / f'readii_{dataset_name}')
            
            readii_index = get_readii_extraction_index(dataset_config=dataset_config,
                                                        readii_index_path=readii_index_path,
                                                        mit_index_path=mit_index_path)
        else:
            message = "READII requires regions and permutations to be set together, or both unset."
            logger.error(message)
            raise ValueError(message)
